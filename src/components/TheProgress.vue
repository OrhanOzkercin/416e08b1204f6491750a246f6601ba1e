<template>
  <section
    class="flex w-full justify-around gap-8 rounded-md bg-slate-700/30 px-1 py-7 md:gap-20 md:py-10 md:px-10"
  >
    <div
      class="text-center"
    ...
    >
      <TheProgressCard :class="{ 'border-primary': activeCard === index }">
        <component
          ...
          :size="32"
          :fillColor="activeCard === index ? '#eab308' : '#fff'"
        />
      </TheProgressCard>
      <span
...
const options = [
  { label: 'Otel ve Tarih', iconComponent: CalendarIcon },
  { label: 'Oda ve Manzara', iconComponent: SleepIcon },
  { label: 'Önizleme ve Ödeme', iconComponent: PaymentIcon }
]
